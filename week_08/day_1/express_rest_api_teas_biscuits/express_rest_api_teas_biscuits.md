...
1. http://localhost:5000/api/teas
2. http://localhost:5000/api/biscuits
...
> We are using `/api` in the routes to distinguish it from our front-end application. This way, if we wanted a second URL, http://localhost:5000/teas, that served HTML, it wouldn't conflict with the API.
...
Now when we visit http://localhost:5000/api/teas in the browser, we see the JSON teas data displaying. We have just made the first endpoint of our API.
...
4. Add the URL (http://localhost:5000/api/teas) in the address bar at the top.
5. Click 'Send' to see the response data display in the right-pane.

![Send GET request](images/1_insomnia_get.png)

*GET request to 'http://localhost:5000/api/teas' displaying the response using Insomnia REST Client*

### Show

Let's now implement the show action, where we respond to the request with one object. The request URL will contain a parameter, which we use to identify which object is being requested. For example, 'http://localhost:5000/api/teas/1', where '1' relates to the id of the tea being requested.
...
3. Add the URL (http://localhost:5000/api/teas/1) in the address bar at the top.
4. Click 'Send' to see the response data display in the right-pane.

![Send GET request](images/2_insomnia_show.png)

*GET request to 'http://localhost:5000/api/teas/1' displaying the tea object at position 1 in the array using Insomnia REST Client*
...
3. Add the URL (http://localhost:5000/api/teas) in the address bar at the top.
...
*POST request to 'http://localhost:5000/api/teas' displaying the response using Insomnia REST Client*
...
1. When the application loads, data is fetched from both the Teas and Biscuits API. This data is filtered down through props to the relevant components.

2. When the form is submitted, `handleSubmit` creates an object using the submitted values and calls `onTeaSubmit` or `onBiscuitSubmit` with this object. These callbacks are passed down as properties from the parent container. The form is then cleared.

3. In the parent container, `handleTeaSubmit` or `handleBiscuitSubmit` carries out a `POST` request, passing the object as a second optional argument, that specifies three properties:
- the method - POST
...
4. When the server responds, we use the `fetchTeas` or `fetchBiscuits` to refresh the data.
